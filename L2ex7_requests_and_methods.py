...
print(f"Ответ при отсутствующем парметре method: {response_one.text}")
wrong_method_answer = response_one.text

response_two = requests.head(link)
print(f"Ответ при изпользовании некорректного типа запроса статус код: {response_two.status_code}")
...
for method_type in dict_of_method:
    for param in dict_of_method:
        ...
        message = f"Некорректный ответ! Метод запроса: {method_type}, " \
                  f"параметр: {formatting_param}, ответ: {request.text}, {request.status_code}"
